...
AIR support for Linux x86_64 and ARM64 is only for provided for developers with a commercial AIR SDK license. 
...
The AIR shared runtime is not supported on Linux. As a result, .air packages cannot be created directly with AIR SDK. Applications should instead be distributed as bundles containing the captive runtime and packaged using native Linux deployment tools.
:::
